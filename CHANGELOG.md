...
- 9a72947: update url `?$tag=` to `$tag[appKey]=`
...
- 05b5108: Make `artifact` input optional to allow source-based publishing from the working directory.
...
- 1401939: Simplify Azure Service Principal authentication by making `azure-resource-id` optional. The action now automatically detects the appropriate Azure Resource ID based on the target environment, reducing required inputs from 3 to 2 Azure credentials (`azure-client-id` and `azure-tenant-id`).
